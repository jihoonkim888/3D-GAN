...
with open('/home/jihoon/diss/data/test.npy', 'rb') as f:
    up_voxels = np.load(f)
    print('voxels shape:', up_voxels.shape)
...
for i in range(up_voxels.shape[0]):
    ...
    print('upscaled')
    m = Mesh.from_voxel_grid(
        up_data, colors=(0.8, 0, 0))
    show(m)

    # l = Lines.from_voxel_grid(up_data, colors=(0, 0, 0.), width=0.01)
    # show([m, l])
